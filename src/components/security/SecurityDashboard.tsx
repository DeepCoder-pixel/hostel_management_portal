        ...
        return (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-bold text-gray-800">Outpass Management</h2>
              <button
            ...
            {/* New Outpass Form Modal */}
            {showOutpassForm && (
              <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
                <div className="bg-white rounded-xl max-w-2xl w-full p-6 max-h-[90vh] overflow-y-auto">
                  <h3 className="text-xl font-semibold mb-6">Record New Outpass</h3>
                  
                  <form onSubmit={handleCreateOutpass} className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => setNewOutpass(prev => ({ ...prev, studentName: e.target.value }))}
                      ...
                      <div>
                        ...
                        <input
                          type="text"
                          value={newOutpass.roomNumber}
                          onChange={(e) => setNewOutpass(prev => ({ ...prev, roomNumber: e.target.value }))}
                          className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                          required
                        />
                      </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">Outpass Type *</label>
                        <select
                          value={newOutpass.outpassType}
                          onChange={(e) => setNewOutpass(prev => ({ ...prev, outpassType: e.target.value }))}
                          className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                          required
                        >
                          <option value="day">Day Outpass</option>
                          <option value="night">Night Outpass</option>
                          <option value="long">Long Outpass</option>
                        </select>
                      </div>
                      <div>
                        ...
                        <input
                          type="time"
                          value={newOutpass.outTime}
                          onChange={(e) => setNewOutpass(prev => ({ ...prev, outTime: e.target.value }))}
                          className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                          required
                        />
                      </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input
                          type="time"
                          value={newOutpass.expectedReturn}
                          onChange={(e) => setNewOutpass(prev => ({ ...prev, expectedReturn: e.target.value }))}
                          className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                          required
                        />
                      </div>
                      <div>
                        ...
                        <input
                          type="text"
                          value={newOutpass.purpose}
                          onChange={(e) => setNewOutpass(prev => ({ ...prev, purpose: e.target.value }))}
                          className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                    ...
                    <div className="flex items-center gap-3">
                      <input
                        type="checkbox"
                        id="parentApproval"
                        checked={newOutpass.parentApproval}
                        onChange={(e) => setNewOutpass(prev => ({ ...prev, parentApproval: e.target.checked }))}
                        className="w-4 h-4 text-purple-600 border-gray-300 rounded focus:ring-purple-500"
                      />
                      <label htmlFor="parentApproval" className="text-sm font-medium text-gray-700">
                        Parent/Guardian Approval Received
                      </label>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        <input
                          type="file"
                          accept="image/*"
                          onChange={(e) => setNewOutpass(prev => ({ ...prev, image: e.target.files?.[0] || null }))}
                          className="hidden"
                          id="outpass-image-upload"
                        />
                        <label
                          htmlFor="outpass-image-upload"
                        ...
                        {newOutpass.image && (
                          <span className="text-sm text-green-600">{newOutpass.image.name}</span>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex gap-3 pt-4">
                      ...
                      <button
                        type="button"
                        onClick={() => setShowOutpassForm(false)}
            ...
            {editingOutpass && (
              <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
                <div className="bg-white rounded-xl max-w-md w-full p-6">
                  <h3 className="text-xl font-semibold mb-6">Mark Student Return</h3>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex gap-3 pt-4">
                      <button
                        onClick={() => handleMarkReturn(editingOutpass)}
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-200">
                    ...
                    <th className="text-left py-3 px-4 font-medium text-gray-700">Purpose</th>
                    <th className="text-left py-3 px-4 font-medium text-gray-700">Out Time</th>
                    <th className="text-left py-3 px-4 font-medium text-gray-700">Expected Return</th>
                    <th className="text-left py-3 px-4 font-medium text-gray-700">Actual Return</th>
                    <th className="text-left py-3 px-4 font-medium text-gray-700">Parent Approval</th>
                    <th className="text-left py-3 px-4 font-medium text-gray-700">Status</th>
                ...
                <tbody>
                  {outpassData.map((outpass) => (
                    <tr key={outpass.id} className="border-b border-gray-100">
                      <td className="py-3 px-4 font-medium text-gray-800">{outpass.studentName}</td>
                      <td className="py-3 px-4 text-gray-600">{outpass.roomNumber}</td>
                      <td className="py-3 px-4 text-gray-600">
                        <span className="capitalize">{outpass.outpassType}</span>
                      </td>
                      <td className="py-3 px-4 text-gray-600">{outpass.purpose}</td>
                      <td className="py-3 px-4 text-gray-600">{outpass.outTime}</td>
                      <td className="py-3 px-4 text-gray-600">{outpass.expectedReturn}</td>
                      <td className="py-3 px-4 text-gray-600">{outpass.returnTime || 'Not returned'}</td>
                      <td className="py-3 px-4">
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                          outpass.parentApproval ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                        }`}>
                          {outpass.parentApproval ? 'APPROVED' : 'NOT APPROVED'}
                        </span>
                      </td>
                      <td className="py-3 px-4">
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${getOutpassStatusColor(outpass.status)}`}>
                          {outpass.status.replace('_', ' ').toUpperCase()}
                        </span>
                      </td>
                      <td className="py-3 px-4">
                        {outpass.status === 'not_returned' && (
                          <button
                            onClick={() => setEditingOutpass(outpass.id)}
